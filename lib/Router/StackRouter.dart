...
class NavigationOption {
  NavigationOption(this.routerConfig);
  ...
  void navigate(BuildContext context, String routerName,
      {Map<String, dynamic> navigationParams = const {}}) {
...
class StackRouterOption {
  StackRouterOption({this.routerName, this.appBar, this.backgroundColor});
...
class RouterConfig {
  RouterConfig({this.screen, this.option});
  final screelBack screen;
  final StackRouterOption option;
}

typedef screelBack = Widget Function(
    NavigationOption navigationOption, Map<String, dynamic> navigationParams);

class Router extends StatelessWidget {
  const Router({Key key, this.routerData, this.optionData}) : super(key: key);
...
// 创建容器页面
Widget createStackRouter(
    {Map<String, RouterConfig> router, Map<String, String> option}) {
